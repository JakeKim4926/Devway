...
import com.ssafy.devway.news.dto.Naver.NaverResponse;
...
public class NewsBlock implements BlockElement {
    ...
    public NewsBlock(String CLIENT_ID, String CLIENT_SECRET_KEY) {
        this.properties = new NewsProperites();

        properties.setClientId(CLIENT_ID);
        properties.setClientSecretKey(CLIENT_SECRET_KEY);
    }

    public void searchNews(String query, SortCondition sort){
    ...
    private void processResponse(ResponseEntity<String> response) throws JsonProcessingException {
        ...
        for (Item item : naverResponse.getItems()) {
            NewsResponse blogResponse = NewsResponse.builder()
                .title(item.getTitle())
                .link(item.getLink())
